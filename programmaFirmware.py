                            ...
                            for i in range(0, recordLength):
                                ...
                                if self.bytesPerAddress == 2:
                                    if (totalAddress+i) < 0x06:
                                        ...
                                        self.pData[j][totalAddress -
                                                      (self.memoryRegion[j].getAddress()*self.bytesPerAddress) + i] = dato

                                else:
                                    self.pData[j][totalAddress-(self.memoryRegion[j].getAddress()
                                                                * self.bytesPerAddress) + i] = dato
        ...
        for currentMemoryRegion in range(0, self.memoryRegionsDetected):
            ...
            while addressToRequest < (self.memoryRegion[currentMemoryRegion].getAddress()+self.memoryRegion[currentMemoryRegion].getSize()):
                ...
                if (addressToRequest+int(self.bytesPerPacket/self.bytesPerAddress)) > (self.memoryRegion[currentMemoryRegion].getAddress()+self.memoryRegion[currentMemoryRegion].getSize()):
                    self.rawData[indice-1] = self.bytesPerPacket-((addressToRequest+int(self.bytesPerPacket/self.bytesPerAddress)-(
                        self.memoryRegion[currentMemoryRegion].getAddress()+self.memoryRegion[currentMemoryRegion].getSize())))*self.bytesPerAddress
        ...
        for currentMemoryRegion in range(0, self.memoryRegionsDetected):
            ...
            while True:
                ...
                if (address+16) > (self.memoryRegion[currentMemoryRegion].getAddress()+self.memoryRegion[currentMemoryRegion].getSize()):
                    bytesThisLine = 16-((address+16)-((self.memoryRegion[currentMemoryRegion].getAddress(
                    )+self.memoryRegion[currentMemoryRegion].getSize())*self.bytesPerAddress))
...
def funzioneMain(fileName = ".//fileOperazioni.json"):
    ...
    if os.path.isfile(fileName):
        ...
        for chiave in chiavi:
            ...
            if datiJSON[chiave]["operazione"] == "erase":
                ...
                rit = sampleDevice.gestisciProgrammazione(
                    bootLoader, datiJSON[chiave]["nomeFile"], datiJSON[chiave]["indSeriale"], ricaricaMem)
                if rit is None:
...
#                convertBinaryToHex(".//temp.bin",datiJSON[chiave]["nomeFile"])
                ...
                ret = sampleDevice.caricaFileHex(
                    bootLoader, datiJSON[chiave]["indSeriale"], datiJSON[chiave]["nomeFile"])
                if ret is None:
